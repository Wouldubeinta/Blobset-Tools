...
namespace OggSharp
{
    public class VorbisFile : IDisposable
    {
        ...
        private readonly SyncState oy = new();
        ...
        public VorbisFile(MemoryStream inst) : this()
        {
        ...
        private static int fseek(MemoryStream fis,
                         //int64_t off,
                         long off,
                         int whence)
        {
        ...
        private static long ftell(MemoryStream fis)
        {
            try
            {
                if (fis.CanSeek == true)
        ...
        private int open(MemoryStream iis, byte[] initial, int ibytes)
        {
            return open_callbacks(iis, initial, ibytes);
        }

        private int open_callbacks(MemoryStream iis, byte[] initial, int ibytes)
        {
            int ret;
            datasource = iis;
        ...
        public void Dispose()
        {
            if (datasource != null) { datasource.Dispose(); datasource = null; }
        }
    }
}
