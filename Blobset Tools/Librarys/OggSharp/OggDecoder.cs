...
namespace OggSharp
{
    ...
    public class OggDecoder : IEnumerable<PCMChunk>
    {
        ...
        private readonly Page og = new(); // one Ogg bitstream page.  Vorbis packets are inside
        ...
        private int[] _index;
        private readonly byte[] convbuffer = new byte[8192];
                        ...
                        while (true)
                        {
                            ...
                            else
                            {
                                ...
                                while ((samples = vd.synthesis_pcmout(_pcm, _index)) > 0)
                                {
                                    ...
                                    {
                                        PCMChunk chunk = new() { Bytes = convbuffer, Channels = vi.channels, Length = bytesRead, Rate = vi.rate };
                                        bytesRead = 0;
                                        yield return chunk;
                                    }
