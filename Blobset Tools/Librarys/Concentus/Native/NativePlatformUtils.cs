...
namespace Concentus.Native
{
    ...
    {
        private static readonly bool _cachedPlatformInfoExists = false;
        private static OSAndArchitecture _cachedPlatformInfo = new(PlatformOperatingSystem.Unknown, PlatformArchitecture.Unknown);
        ...
        internal static NativeLibraryStatus PrepareNativeLibrary(string libraryName, TextWriter logger)
        {
            ...
            lock (_mutex)
            {
                ...
                {
                    DirectoryInfo probeDir = new(Path.Combine(baseDirectory, possibleDirectory, "native"));
                    if (!probeDir.Exists)
                    {
                        continue;
                    }
                    ...
                    {
                        FileInfo sourceLibraryFile = new(Path.Combine(probeDir.FullName, possibleSourceLibrary));
                        ...
                        {
                            FileInfo desiredBinplacePath = new(Path.Combine(Environment.CurrentDirectory, normalizedLibraryName));
                            try
                            {
        ...
        {
            FileInfo existingLocalLibPath = new(Path.Combine(Environment.CurrentDirectory, normalizedLibraryName));
        ...
        private static List<string> PermuteArchitectureSpecificDirectoryNames(OSAndArchitecture platformInfo)
        {
            ...
            List<string> returnVal = new(inheritedRids.Length + 1);
