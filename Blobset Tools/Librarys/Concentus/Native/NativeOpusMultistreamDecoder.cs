...
namespace Concentus.Native
{
    ...
    internal class NativeOpusMultistreamDecoder : IOpusMultiStreamDecoder
    {
        ...
        public static unsafe NativeOpusMultistreamDecoder Create(int sampleRate, int channelCount, int streams, int coupledStreams, byte[] channelMapping)
        {
            fixed (byte* mappingPtr = channelMapping)
            {
                int error;
                ...
                NativeOpusMultistreamDecoder returnVal = new(handle);
                returnVal._sampleRate = sampleRate;
                returnVal._numChannels = channelCount;
                return returnVal;
            }
